...
async def test_orchestrator_saves_log():
    ...
    with patch(
        "bridge_app.services.orchestrator.get_monarch_client", new_callable=AsyncMock
    ) as mock_get_client, patch(
        "bridge_app.services.orchestrator.push_transaction", new_callable=AsyncMock
    ) as mock_push:
        ...
        with patch(
            "bridge_app.services.currency.get_exchange_rate", new_callable=AsyncMock
        ) as mock_rate:
            mock_rate.return_value = 1.10

            await _process_transaction_data(
                data, "hash123", mock_db, mock_report, force_override=True
            )
            ...
            print(
                "✅ SUCCESS: Transaction successfully logged to database with conversion"
            )
...
async def test_update_date_endpoint_updates_log():
    ...
    with patch(
        "bridge_app.main.get_monarch_client", new_callable=AsyncMock
    ) as mock_get_client, patch(
        "bridge_app.services.monarch.update_transaction_fields", new_callable=AsyncMock
    ) as mock_update_fields, patch(
        "bridge_app.services.currency.get_exchange_rate", new_callable=AsyncMock
    ) as mock_rate:
        ...
        await update_transaction_date(app_req, db=mock_db)
